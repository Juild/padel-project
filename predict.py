...
for image, box in predict_dataloader:
    ...
    # result : image.shape -> (1080, 1920, 3)
    image = image.cpu()\
        .squeeze()\
        .permute(2, 1, 0)\
        .numpy()
    ...
    x0 = int(predicted_box[0] * image.shape[0])
    y0 = int(predicted_box[1] * image.shape[1])
    x1 = int(predicted_box[2] * image.shape[0])
    y1 = int(predicted_box[3] * image.shape[1])
    print(x0, y0, x1, y1)
    print(predicted_box)
    image = cv2.rectangle(image, (x0, y0), (x1, y1), (0, 0, 255), 20)
